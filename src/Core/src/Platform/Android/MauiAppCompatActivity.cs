...
namespace Microsoft.Maui
{
	public partial class MauiAppCompatActivity : AppCompatActivity
	{
		...
		protected override void OnCreate(Bundle? savedInstanceState)
		{
			...
			// Register MauiOnBackPressedCallback BEFORE CreatePlatformWindow so that any
			// callbacks added during window/handler creation (e.g. BlazorOnBackPressedCallback)
			// sit on top of the LIFO stack and fire first, as intended.
			// Use OnBackPressedCallback (AndroidX) so the system predictive back-to-home
			// animation plays when the app has nothing to handle (Enabled = false).
			// IOnBackInvokedCallback (Android 13+ API) was avoided here because registering
			// one always suppresses the back-to-home animation regardless of Enabled.
