		...
		Size EnsureContentSizeForScrollDirection(double widthConstraint, double heightConstraint, Size contentSize)
		{
				...
				(scrollDirection == UICollectionViewScrollDirection.Horizontal && contentSize.Width == 0))
			{
				...
				if (collectionView.Window == null)
				{
					...
					try
					{
						// CGRect dimensions must be finite. When called with double.PositiveInfinity
						// (unconstrained measure), clamp to UILayoutFittingExpandedSize (10000) so UIKit
						// can run a full layout pass. The actual computed content size is the output.
						var frameWidth = double.IsInfinity(widthConstraint) ? 10000 : widthConstraint;
						var frameHeight = double.IsInfinity(heightConstraint) ? 10000 : heightConstraint;

						// Give the CollectionView the available size so the layout calculates correctly
						collectionView.Frame = new CoreGraphics.CGRect(0, 0, (nfloat)frameWidth, (nfloat)frameHeight);
